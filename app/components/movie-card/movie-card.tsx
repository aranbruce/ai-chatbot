...
const MovieCard = ({title, description, imdbRating, releaseYear, director, genre, stars, imageURL}: MovieCardProps) => {
  return (
    ...
    {imageURL && (<Image className="rounded-md" src={imageURL} width={80} height={117} alt={title} />)}
      ...
      <div className="flex flex-col gap-2 text-sm text-zinc-700 dark:text-zinc-400">
        ...
        <p>Stars: </p>
          {stars.map((star: string, index: number) => (
            <span key={index}>{star}{index < stars.length - 1 && ", "}</span>
          ))}
        </div>
      </div>
    </div>
