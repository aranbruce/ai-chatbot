...
const UploadButton = () => {
  const { fileCollection, setFileCollection, filesAsInput, setFilesAsInput } = useContext<FileCollectionContextProps>(FileCollectionContext);
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      .then(data => {
        const fileObject: FileCollectionItem[] = [{ fileId, fileName: file.name, fileContent: data }];
        setTimeout(() => {
  ...
  return (
    <div className={`display flex flex-wrap gap-1 items-start ${filesAsInput.length > 0 ? "p-2" : ""}`}>
      {filesAsInput && filesAsInput.map((file) => (
        <div key={file.fileId} className={"display flex flex-col gap-2 items-start transition"}>
          <div className="flex flex-row relative  bg-white dark:bg-zinc-900 shadow-sm left-0 border border-zinc-200 dark:border-zinc-700 p-3 rounded-lg text-sm  text-zinc-500 dark:text-zinc-400 text gap-2">
            ...
            <button className="absolute top-[-12px] right-[-12px] rounded-full bg-zinc-100 dark:bg-zinc-700 p-[3px] border border-zinc-300 dark:border-zinc-700 text-zinc-900 dark:text-zinc-50" onClick={() => handleRemoveFileButtonClick(file.fileId)}>
      ...
      <button type="button" aria-label="attach file" className="flex bg-white dark:bg-zinc-800 rounded-full border border-zinc-200 dark:border-zinc-800 p-[0.375rem] items-center h-fit bottom-[0.75rem] left-[0.75rem] absolute text-zinc-900 dark:text-zinc-50" onClick={() => handleAddFileButtonClick()}>
