...
export async function GET(request: NextRequest) {
  ...
  const freshnessOptions = ["past-day", "past-week", "past-month", "past-year"];
  ...
  if (freshness && !freshnessOptions.includes(freshness)) {
    return NextResponse.json(
      { error: "Invalid freshness option" },
      { status: 400 }
    );
  ...
  // call brave API
  const url =
    `https://api.search.brave.com/res/v1/web/search?q=${query}&text_decorations=0` +
    `&count=${count}` +
